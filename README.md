...
1. 在**背包**标签页中，使用分类复选框和筛选框查找物品。
2. 点击要修改的物品将其添加到右侧列表，可设置**数量**，武器/护甲可选择**稀有度**。
3. 修改完物品的属性后点击**应用修改**。注意：每件物品修改完都要**应用修改**，目前无法一次性批量应用修改。
4. 点击角色名旁边的**应用**，将数据应用到选择的角色。
5. 点击**保存**，然后读取游戏。修改的物品会出现在该角色的背包中。
...
## 🔧 从源码编译
- 请看[urbanpabs分支里的说明](https://github.com/urbanpabs/D-OS-Save-Editor)
